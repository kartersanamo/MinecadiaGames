...
class _LevelingManagerCore:
    ...
    async def _get_user_stats(self, user_id: int) -> Optional[Dict]:
        """Get user's current leveling stats from database"""
        db = await self.db
        rows = await db.execute(
            "SELECT user_id, xp, level, is_active, ever_played FROM leveling WHERE user_id = %s",
            (user_id,),
        )
        return rows[0] if rows else None

    async def _create_user_entry(self, user_id: int) -> Dict:
        """Create a new user entry in the leveling table"""
        db = await self.db
        await db.execute_insert(
            "INSERT INTO leveling (user_id, xp, level, is_active, ever_played) VALUES (%s, %s, %s, %s, %s)",
            (user_id, 0, 1, 0, 0),
        )
        return {"user_id": user_id, "xp": 0, "level": 1, "is_active": 0, "ever_played": 0}
